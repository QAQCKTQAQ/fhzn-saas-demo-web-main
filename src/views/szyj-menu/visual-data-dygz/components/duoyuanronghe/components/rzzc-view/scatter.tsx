...
export default function Scatter(props: any) {
	...
	const handleDataByType = () => {
		const { maichong_shijians = [], fudus, maichong_pinlvs, maichong_kuandus } = inputData || {}
		if (type === CHART_TYPE_ENUM.gz_pl) {
			return (maichong_shijians || []).map((item: any, index: number) => {
				return [item, maichong_pinlvs[index]]
			})
		}
		if (type === CHART_TYPE_ENUM.mc_kd) {
			return (maichong_shijians || []).map((item: any, index: number) => {
				return [item, maichong_kuandus[index]]
			})
		}
		if (type === CHART_TYPE_ENUM.fu_du) {
			return (maichong_shijians || []).map((item: any, index: number) => {
				return [item, fudus[index]]
			})
		}
		if (type === CHART_TYPE_ENUM.jd_wd) {
			return [
				[0, 8.04],
				[8.07, 6.95],
				[13.0, 7.58],
