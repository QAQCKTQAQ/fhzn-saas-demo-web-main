import { Form, Row, Col, Space, Button, Input, message } from 'antd'
...
import { jsonToCanvasData, txtToCanvasData } from '../../const'
const FileInfo = () => {
	...
	return (
		<div>
			...
			<Form form={form} style={{ width: '100%' }} labelCol={{ style: { width: 80 } }} labelAlign={'left'} preserve={false}>
				<Row>
					<Col span={24}>
						<Form.Item label={'名称'}>{renderEditName()}</Form.Item>
					</Col>
					<Col span={24}>
						<Form.Item label={'格式'}>{`${fileType || ''}`.slice(1).toLocaleUpperCase()}</Form.Item>
					</Col>
					<Col span={24}>
					...
					<Col span={24}>
						<Form.Item label={'分辨率'}>
							{`${fileDetailData?.fileContent?.width || '-'} * ${fileDetailData?.fileContent?.height || '-'}`}
						</Form.Item>
					</Col>
					<Col span={24}>
						...
						<Form.Item label="标注信息">
							<Space direction="vertical" style={{ width: '100%' }}>
								<div className="label-btn">
									...
									<DownloadOutlined style={{ visibility: 'hidden' }} />
									<FileTextOutlined style={{ visibility: 'hidden' }} />
								</div>
