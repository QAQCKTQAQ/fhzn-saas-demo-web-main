...
function UploadModal(props: any) {
	...
	const submit = async () => {
		setLoading(true)
		const { id, isAnnotation } = currentData.current.record
		const fileNoList = fileList.filter((item: any) => item.done === 'success').map((item: any) => item.code)
		importFileApi({ fileCodes: fileNoList, id, isAnnotation })
			.then(() => {
